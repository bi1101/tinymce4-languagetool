...
var LanguageTool = function LanguageTool(lt_set, $) {
    ...
    function lt_applyHighlighter(myobj, myid, myword, myrule, myclass, mystart, myend, suggestions, message, shortMessage) {
        ...
        // Constructing the initial part of the HTML structure using a template literal
        var suggestionHTML = `
        <div class="upgrade_vocab">
            <span class="original-vocab">${myword}</span>
            <span class="arrow">-></span>`;

        // Iterate over each suggestion to create its improved-vocab structure
        suggestions.forEach(function (suggestion) {
            suggestionHTML += `<span class="improved-vocab">${suggestion.value}</span>`;
        });

        // Completing the HTML structure
        suggestionHTML += `
        <span class="short-explanation"> · ${shortMessage || message || ''}</span>
        <br>
        <span class="explanation">${message || shortMessage || ''}</span></div>`;

        // Append the HTML structure to the suggestions panel
