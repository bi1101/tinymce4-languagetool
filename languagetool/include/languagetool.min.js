...
var LanguageTool = function LanguageTool(lt_set, $) {
    ...
    // This function is triggered when the selected node changes
    sgevent['nodechange'] = function (e) {
        // If the mode is not set to 'manual'
        if (lt_setMode != 'manual') {
            // Resume the language tool if it's idle
            lt_timer(0);
            ...
            // If the selected node matches the ignore selector, exit the function
            ...
            // Log the node name and UUID of the selected node for debugging
            lt_debug('TRACE|NODECHANGE=' + mynode.nodeName + '=' + uuid, 6);

            // If the id of the parent node is 'mcepastebin', exit the function
            if (mypnode.id == 'mcepastebin') {
                return;
            }

            // If the parent node doesn't have a UUID
            if (uuid == undefined) {
                // Assign a new UUID to the parent node
                uuid = setUUID(mypnode);
            }

            // If the parent node is not the last node
            if (!$(mypnode).is(lastnode)) {
                // Get the UUID of the last node
                var lastuuid = $(lastnode).data('UUID');

                // Add the last node to the queue
                lt_queue(lastuuid, lastnode, 0, 0);

                // Update the last node to be the current parent node
                lastnode = mypnode;
            }
        }
    };
    ...
    sgevent['keypress'] = function (e) {
        ...
        if (lt_setMode != 'manual') {
            ...
            if (key == 46  // .
                ...
                // Add the parent node to the queue to be processed
                lt_queue(uuid, mypnode, 0, 1);
    ...
    function lt_queue(uuid, mynode, qlvl, newline) {
        // Check if 'enabled' property of sgstore is true and uuid is not undefined
        if (sgstore['enabled'] == true && uuid != undefined) {
            // Get the text and HTML of mynode
            var thtemp = lt_textHTML(mynode);
            // Store the text in sgcheck with uuid as the key
            sgcheck[uuid] = thtemp['text'];
            // Store the HTML in sgstore with uuid as the key
            sgstore['html'][uuid] = thtemp['html'];
            // Store the mynode object in sgstore with uuid as the key
            sgstore['obj'][uuid] = mynode;
            // If viewport_process of sgstore is 'disabled', show progress bar
            if (sgstore.viewport_process == 'disabled') { lt_progress_bar(0, 3); }
            // Push the uuid into the queue of sgstore at level qlvl
            sgstore['queue'][qlvl].push(uuid);
            // If newline is greater than 0, set newLineCheck to newline
            if ((newline || 0) > 0) {
                newLineCheck = newline;
            }
            // Call lt_timer function with 0 as argument to resume if idle
            lt_timer(0);
        }
    }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        sgstore['run'].push({
                            ...
                            func: function () {
                                // Call lt_progress_bar with this.tsize as argument
                                lt_progress_bar(this.tsize);

                                // Store the data in json and the text in mytext
                                var json = this.data;
                                var mytext = this.text;

                                // Initialize uuidcount and bm
                                var uuidcount = {};
                                var bm = sgfunc['bookmark'].call();

                                // Replace all occurrences of the regex pattern in mytext
                                mytext.replace(/\t\(([a-f0-9-]{36})\)\n\n/, function (pmatch, p1) {
                                    var tempid = p1;

                                    // Get the HTML of the object with key tempid in 'obj' of sgstore, clear the highlighter, and store it in temphtml
                                    var temphtml = lt_clearHighlighter(lt_text($(sgstore['obj'][tempid]).html()));

                                    // Set the HTML of the object with key tempid in 'obj' of sgstore to temphtml
                                    $(sgstore['obj'][tempid]).html(temphtml);

                                    return pmatch;
                                });

                                // Initialize fmatch and locklist
                                var fmatch = {};
                                var locklist = {};
                                ...
                                for (var i = 0; i < tlen; i++) {
                                    ...
                                    // If the issueType of rule in item is defined in lt_setHighlight, set hclass to it
                                    if (lt_setHighlight[item['rule']['issueType']] != undefined) {
                                        hclass = lt_setHighlight[item['rule']['issueType']];
                                    }

                                    // Get the substring of mytext starting at the offset of item and store it in tempstr
                                    var tempstr = mytext.substr(item['offset']);

                                    // Initialize uuid, templen, and tempword
                                    var uuid;
                                    var templen;
                                    var tempword;

                                    // Get the substring of tempstr with length equal to the length of item and store it in tempword
                                    var tempword = tempstr.substr(0, item['length']);

                                    // Replace the first occurrence of the regex pattern in tempstr and store the second capturing group in uuid
                                    tempstr.replace(/(?:^|\n)(.*?\t)\(([a-f0-9-]{36})\)\n/, function (pmatch, p1, p2) { uuid = p2; return pmatch; });

                                    // Replace the first occurrence of the regex pattern in mytext and store the index of the first capturing group in templen
                                    ...
                                    // If uuid is defined and tempword is not in 'all' or 'once' of 'ignore' in sgstore, proceed
                                    if (uuid != undefined && sgstore['ignore']['all'][tempword] == undefined && (sgstore['ignore']['once'][uuid] == undefined || sgstore['ignore']['once'][uuid][tempword] == undefined)) {
                                        ...
                                        // Subtract templen from the offset of item
                                        item['offset'] -= templen;
                                        lt_debug("TRACE|OFFSET=" + item['offset'] + '=' + templen, 6);

                                        // If uuid is not in fmatch, initialize it in 'lt' of sgstore and set it in fmatch
                                        if (fmatch[uuid] == undefined) {
                                            sgstore['lt'][uuid] = {};
                                            fmatch[uuid] = 1;
                                        }

                                        // Store item in 'lt' of sgstore with keys uuid and i, and set 'word' in it to tempword
                                        sgstore['lt'][uuid][i] = item;
                                        sgstore['lt'][uuid][i]['word'] = tempword;

                                        // Apply the highlighter to the object with key uuid in 'obj' of sgstore
                                ...
                                // For each key-value pair in locklist, call lt_lock with arguments 0 and key
                                $.each(locklist, function (key, val) {
                                    lt_lock(0, key);
                                });

                                // Call the bookmark function of sgfunc with bm as argument
                                sgfunc['bookmark'].call(this, bm);

                                // Delete the key-value pair with key this.ikey in listInstances
    ...
    function lt_applyHighlighter(myobj, myid, myword, myrule, myclass, mystart, myend, suggestions, message, shortMessage) {
        ...
        <br>
        <span class="explanation">${message || shortMessage || ''}</span></div>`;
    ...
    /**
     * Processes new content for LanguageTool.
     *
    ...
    function lt_contextMenuClick(uuid, hid, value, mode) {
        ...
        if (mynode.nodeName == 'SPAN' && $(mynode).hasClass('lt-highlight') == true) {
            // Push an object to sgstore['run'] with the node, value, mode, and a function
            sgstore['run'].push({
                node: mynode,
                value: value,
                mode: mode,
                func: function () {
                    ...
                    // Add the parent of mynode to the queue with the UUID
